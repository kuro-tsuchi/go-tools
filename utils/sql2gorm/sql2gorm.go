...
func MainExec(sql string) (interface{}, error) {
	data, err := parser.ParseSql(sql, parser.WithTablePrefix(""), parser.WithJsonTag())
	if err != nil {
		return nil, fmt.Errorf("parseSql error %v", err)
	}
	ret := data.StructCode[0]
	fmt.Printf("parseSql result:\n%v\n", ret)
	return ret, nil
}
